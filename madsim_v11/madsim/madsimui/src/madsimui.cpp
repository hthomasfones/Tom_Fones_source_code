...
//MADREGS MadRegs = {0, 0, 0, 0x080B0B, 0};
...
int op, parm, val;
    ...
    if (rc != 0)
        {fprintf(stderr, "madsimui returning rc=%d\n", rc);}
...
MADBUSCTLPARMS MadBusCtlParms;
	...
	case kINI: //Init, ???
	case kRST:
	    rc = ioctl(fd, MADBUS_IOC_RESET, NULL);
	    break;

        case kHPL:
            MadBusCtlParms.Parm = parm; //devnum (slotnum) 
            MadBusCtlParms.Val  = val;  //pci_devid
		    rc = ioctl(fd, MADBUS_IOC_HOT_PLUG, &MadBusCtlParms);
			break;
 
	    case kHUN:
		    rc = ioctl(fd, MADBUS_IOC_HOT_UNPLUG, parm);
			break;
		...
		rc = ioctl(fd, MADBUS_IOC_EXPIRE, NULL);
		break;
	    ...
	    case kGET: //The whole device
		 rc = ioctl(fd, MADBUS_IOC_GET_DEVICE, &MadBusCtlParms);
		if (rc == 0)
...
int Xfer_Device_Data(u8* pDevData, char* file_prefx, int devnum, size_t DataLen, u8 bWrite)
{
    ...
    if(iocount < 0)
        ...
        {fprintf(stderr, "I/O complete to file: (%s) %d bytes transferred from %px\n",
