...
#else
static inline struct block_device *
maddevb_blkdev_get_by_dev(dev_t dev, fmode_t mode, void *holder)
{
    struct block_device *bdev = NULL;
	struct bdev_handle  *hbdev = NULL;
    int err;

    hbdev = bdev_open_by_dev(dev, mode, holder, NULL);
	if (hbdev != NULL)
        {bdev = hbdev->bdev;} //blkdev_get_no_open(dev);
    if (!bdev)
...
static inline void maddevb_blkdev_put(struct bdev_handle *hbdev, fmode_t mode)
{
