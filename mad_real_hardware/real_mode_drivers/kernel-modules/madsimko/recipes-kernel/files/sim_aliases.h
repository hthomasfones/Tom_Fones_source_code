...
#define  pci_alloc_irq_vectors  pcisim_alloc_irq_vectors
#undef   pci_free_irq_vectors
#define  pci_free_irq_vectors  pcisim_free_irq_vectors

#undef   pci_iomap
#define  pci_iomap              pcisim_iomap
#undef   pci_iounmap
#define  pci_iounmap            pcisim_iounmap
...
extern int    sim_register_device(struct device* pDevice, u32 devnum, char* devname);
extern void   sim_unregister_device(struct device* pDevice, u32 devnum);
...
extern void __iomem* 
pcisim_iomap(struct pci_dev *dev, int bar, unsigned long max);
extern void pcisim_iounmap(struct pci_dev *dev, void __iomem* pdevaddr);

extern struct pci_dev*
pcisim_get_device(unsigned int vendor, unsigned int device, struct pci_dev *from);

extern void sim_init_sysfs_device(struct device *pDevice, int devnum, 
                                  char* devname, 
                                  struct device_driver *pdriver, void* drvrdata,
                                  void *release_fn);
...
extern int    pcisim_read_config_dword(const struct pci_dev *dev, int where, uint32_t* val);
...
extern int  __must_check sim_request_irq(unsigned int irq, 
...
extern const void *free_irq(unsigned int irq, void *dev_id);
#endif

extern int pcisim_alloc_irq_vectors(struct pci_dev *pdev, unsigned int min_vecs, 
		                            unsigned int max_vecs, unsigned int flags);
extern void pcisim_free_irq_vectors(struct pci_dev *pdev);
