...
static struct pci_driver maddev_driver =
{
    ...
    .driver = 
         //{.name = DRIVER_NAME, .owner = THIS_MODULE, .p = &maddrvr_priv_data,},
...
int maddev_probe(struct pci_dev *pcidev, const struct pci_device_id *ids)
{
	...
	rc =  maddev_setup_device(pmaddev, &pmaddev->pPcidev, true, bMSI);
    if (rc == 0)
        maddev_nbr_devs++;
    return rc;
}
    ...
    U8 bHPL, u8 bMSI)
{
    ...
    #ifdef _CDEV_
        snprintf(pmaddev->devname, sizeof(pmaddev->devname), "maddevc_obj%d", (int)i);
    #endif
    #ifdef _BLOCKIO_
        snprintf(pmaddev->devname, sizeof(pmaddev->devname), "maddevb_obj%d", (int)i);
    #endif
    
    num_irqs = (bMSI) ? MAD_NUM_MSI_IRQS : 0;
    num_irqs++;
    rc = maddev_claim_pci_resrcs(pPcidevTmp, pmaddev, pmaddev->devname, num_irqs);
    if (rc != 0)
		{
        mutex_unlock(&pmaddev->devmutex);
        PERR("maddev_setup_device... dev#=%u rc=%d\n",
		     (int)i, rc);
		return rc;
		}
    ...
    rc = register_device(pdev,i, devname); //MadDevNames[i]
    #else
    ...
    if (rc != 0)
        ...
        #else
        device_unregister(pmaddev->pdevnode);
        #endif
...
void maddev_remove_device(PMADDEVOBJ pmaddev)
{
   ...
   if (pmaddev->pclassdev != NULL)
    ...
    if (pmaddev->bDevRegstrd)
        ...
        #else
        device_unregister(&pmaddev->pPcidev->dev);
        #endif
        pmaddev->bDevRegstrd = false;
        }
    else
                            ...
                            char* DevName, u16 NumIrqs)
{
    ...
    __iomem u8* bar2 = NULL;
    __iomem u8* bar4 = NULL;
    #endif
    ...
    PINFO("maddev_claim_pci_resrcs:pci_resource_start... dev#=%d, BaseAddr=x%llX BCDPP=x%llX CachePA=x%llX\n",
    ...
    if (rc == 0)    
        {PciCnfgU64_0 = ((U64)((U64)PciCnfgHi << 32) + (PciCnfgLo & 0xFFFFFFF0));}
    ...
    if (rc == 0)
        {PciCnfgU64_2 = ((U64)(((U64)PciCnfgHi << 32) + (PciCnfgLo & 0xFFFFFFF0)));}
    ...
    if (rc == 0)
        {PciCnfgU64_4 = ((U64)(((U64)PciCnfgHi << 32) + (PciCnfgLo & 0xFFFFFFF0)));}

    if (rc != 0)
        {PWARN("maddev_claim_pci_resrcs:pci_read_config_dword... dev#=%d rc=%d\n",
               (int)pmaddev->devnum, rc);}
    
    PINFO("maddev_claim_pci_resrcs:pci_read_config_dword(s)...\n dev#=%d PciCnfgU64_0=x%llX PciCnfgU64_2=x%llX PciCnfgU64_4=x%llX\n",
	...
	if (pmadregs != NULL)
        PINFO("maddev_claim_pci_resrcs:pci_iomap... dev=%d bar0_kva=%px\n",
              (int)pmaddev->devnum, pmadregs);
    else
        {
        rc = -ENOMEM;
    ...
    #ifndef _MAD_SIMULATION_MODE_   
    //#Sanity check: poke the bars on real hardware_   
    maddev_poke_bar(pmadregs, 0xbadf00d);
    bar2 = pci_iomap(pPcidev, 2, 64);
    bar4 = pci_iomap(pPcidev, 4, 64);
    maddev_poke_bar(bar2, 0xdeadbeef);
    maddev_poke_bar(bar4, 0xfeeda11);
    pci_iounmap(pPcidev, bar2);
    pci_iounmap(pPcidev, bar4);
    #endif
...
int maddev_release_pci_resrcs(PMADDEVOBJ pmaddev)
{
    ...
    rc = (free_irq(pmaddev->base_irq, pmaddev) == NULL);
    #endif
    if (pmaddev->pPcidev->msi_cap != 0)
        ...
        for (j=1; j <= (MAD_NUM_MSI_IRQS); j++)
            ...
            #endif
            #if LINUX_VERSION_CODE >= KERNEL_VERSION(6,6,0)
            rc = (free_irq((pmaddev->base_irq+j), pmaddev) == NULL);
            #endif  
            }
    ...
    pci_disable_device(pmaddev->pPcidev);
                               ...
                               phys_addr_t HostAddr, U32 offset, bool bH2D)
{
    ...
    //Only set the index register on sector-sized direct i/o
                              ...
                              U32* io_f, spinlock_t* plock)
{
    ...
    rc = wait_event_interruptible_timeout(*io_q, (*io_f != eIoPending), 
                                          msecs_to_jiffies(MAD_IO_TIMEOUT));
    if (rc == 0) //Timeout
       {
       PWARN("mmaddev_wait_get_io_status()... dev#=%u I/O timeout\n", 
             pmaddev->devnum);
       return -ETIMEDOUT;
       }     
