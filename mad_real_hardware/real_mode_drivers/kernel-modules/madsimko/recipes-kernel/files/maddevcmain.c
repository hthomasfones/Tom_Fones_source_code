...
int maddev_major = 0; //MADDEV_MAJOR_CHARDEV;
...
static struct pci_device_id mad_pci_ids[] =
{
...
//
MODULE_DEVICE_TABLE(pci, mad_pci_ids);
//
...
long maddevc_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	PMADCTLPARMS    pCtlParms = (PMADCTLPARMS)(uintptr_t)arg;
...
static int maddevc_init_module(void)
{
    ...
    dev_t  cdev = 0;
	int    devcount = 0;
    int num_static_devs = maddev_nbr_devs;
    U32    len;
...
//  major number unless directed otherwise at load time. 
    ...
    if (num_static_devs > 0) //# preset static devices
        devcount = maddev_setup_devices(num_static_devs, false, bMSI); 

#ifdef MADDEVOBJ_DEBUG /* only when debugging */
	maddev_create_proc();
#endif

    if (maddev_nbr_devs == 0) //No static or discovered devices
        {rc = 0;} 
    else //Did at least one expected static device init succeed
        {
        if (num_static_devs == 0) //No static devices
            rc = 0;
        else  
            rc = (devcount > 0) ? 0 : -ENOMEDIUM;
        }
 
    if (rc == 0)
        {
        if (maddev_nbr_devs > devcount)
            devcount = maddev_nbr_devs;    
