...
//
	__iomem MADREGS *pmadregs = pmadbusobj->pmadregs;
	__iomem U8*      pDevData = 
                     (__iomem u8*)((U64)pmadregs + MAD_DEVICE_DATA_OFFSET);
    ...
    u32 CacheIndx = 0;
 	...
 	if (write)
		...
		pDevData += ((u64)(CacheIndx * MAD_CACHE_SIZE_BYTES));
        ...
        if (Status & MAD_STATUS_WRITE_CACHE_EMPTY_BIT)
	        ...
	        memcpy_toio(pDevData, bouncebufr, MAD_CACHE_SIZE_BYTES);
 			iowrite32((CacheIndx + MAD_CACHE_NUM_SECTORS), 
                       &pmadregs->CacheIndxWr);
		    }
	    }
	else //cache read
	    {
