...
// Sanity check for real hardware
static inline int maddev_poke_bar(void __iomem *barx, u32 wval)
{
    //static u32 writeval = 0xdeadbeef;
    u32 readval0 =  0;
    u32 readval1 =  0;
    int rc = 0;
 
    ASSERT(barx != NULL);
    readval0 =  ioread32(barx);
    iowrite32(wval, barx);
    readval1 = ioread32(barx);
    rc = (readval1 == wval) ? 0 : -EIO; 
    PINFO("maddev_poke_bar... bar=%px read,write,read = x%x x%x x%x rc=%d\n",
          barx, readval0, wval, readval1, rc);  
...
static inline void 
maddev_program_io_regs(__iomem MADREGS *pmadregs, ULONG ControlReg, 
                       ULONG IntEnableReg, dma_addr_t HostAddr)
{
    u8 __iomem *host_pa = (u8 __iomem *)&pmadregs->HostPA;

    iowrite32(0, &pmadregs->IntID);
    (void)ioread32(&pmadregs->IntID);

    iowrite32(IntEnableReg, &pmadregs->IntEnable);
    (void)ioread32(&pmadregs->IntEnable);

    iowrite32(lower_32_bits(HostAddr), host_pa);
    (void)ioread32(host_pa);

    iowrite32(upper_32_bits(HostAddr), host_pa + 4);
    (void)ioread32(host_pa + 4);

    iowrite32(ControlReg, &pmadregs->Control);
    (void)ioread32(&pmadregs->Control);
}
