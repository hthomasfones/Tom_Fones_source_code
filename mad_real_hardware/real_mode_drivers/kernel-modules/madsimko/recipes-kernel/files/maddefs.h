...
#define MADDEV_MAJOR_CHARDEV   0x1FF //(LINUX_LAB_RESERVED_BASE+1)
#define MADDEV_MAJOR_BLOCKDEV  (MADDEV_MAJOR_CHARDEV-1) //(LINUX_LAB_RESERVED_BASE+2)
//
...
#else
    #define MADDEV_NBR_DEVS   0 //No static devices in real-mode
#endif
...
#define WARN_ON_MACRO(x)     \
        do {                 \
           if (unlikely(x))  \
               ...
               {pr_warn("WARN_ON triggered at %s:%d", __FILE__, __LINE__); dump_stack();} \
           } while (0)
...
#define ASSERT(x) if (!x)  \
        {printk(KERN_WARNING "ASSERT *!* %s, %d\n",__FILE__,__LINE__); WARN_ON_MACRO(1);}    
         //dump_stack();   
         //WARN_ON_MACRO(1);}
...
//
#define MAD_BAR0_APERTURE_SIZE       0x4000 //(PAGE_SIZE * 4) 16KB to match the vivado implementation
#define MAD_REGISTER_BLOCK_SIZE      PAGE_SIZE //(MAD_SECTOR_SIZE * 4)
#define MAD_SIZEOF_REGISTER_BLOCK    MAD_REGISTER_BLOCK_SIZE

#define MAD_MAPD_READ_OFFSET         MAD_BAR0_APERTURE_SIZE //MAPD as in mapped
...
#define MAD_DMA_CNTL_H2D_BIT     0x80000000 //Indicates host to device (dma-write)
