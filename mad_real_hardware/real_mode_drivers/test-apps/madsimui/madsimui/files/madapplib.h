...
void DisplayDevRegs(PMADREGS pmadregs)
{
    ...
    host_lo = *((volatile uint32_t *)
                ((volatile uint8_t *)&pmadregs->HostPA + 0));

    host_hi = *((volatile uint32_t *)
                ((volatile uint8_t *)&pmadregs->HostPA + 4));

    //host_pa = ((uint64_t)host_hi << 32) | host_lo;
    ...
    fprintf(stderr,
            "ReadDX=x%X  WriteDX=x%X  ReadCacheDx=x%X  WriteCacheDx=x%X  HostPA=x%X:%X\n",
