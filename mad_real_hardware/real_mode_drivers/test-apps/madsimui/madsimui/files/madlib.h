...
void DisplayDevRegs(PMADREGS pmadregs)
{
    ...
    fprintf(stderr, 
            "Devnum=x%X MesgID=x%X Control=x%X Status=x%X IntEnable=x%X IntID=x%X\n",
    ...
    fprintf(stderr,
            "ReadDX=x%X  WriteDX=x%X  ReadCacheDx=x%X  WriteCacheDx=x%X  HostPA=%p x%X:%X\n",
	    ...
	    (void *)pmadregs->HostPA, (ULONG)(pmadregs->HostPA >> 32), 
	    (ULONG)(pmadregs->HostPA & 0x00000000FFFFFFFF));
