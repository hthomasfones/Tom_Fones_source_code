...
cd $busdrvrpath
/sbin/insmod ./$busmodule.ko madbus_nbr_slots=$number_bus_slots $* || exit 1

set +x ; printf "\nCreate symbolic links between... \n" ; set -x
#Create symbolic link: soft, force, verbose
#ln -sfv ${busdevobj} /dev/${busdevobj}
ln -sfn /dev/${busdevobj}"0" /dev/${busdevobj}
#
...
if [ -z "$maddev_major" ] || [ "$maddev_major" -eq 0 ]
then
...
lsmaddev=$(lsmod | grep -E '^maddev| maddev')
#
if [ -z "$lsmaddev" ];
then
