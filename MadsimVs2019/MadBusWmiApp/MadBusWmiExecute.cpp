...
/*                                                                             */
/*  Module  NAME : MadBusWmiExecute.cpp                                        */
/*                                                                             */
...
const BSTR className   = SysAllocString(MADBUS_MAD_CONTROL_CLASS);
...
//char x;
    ...
    do {// Get the instance object for each instance of the class.
		...
		printf_s("instanceObj->Get returns: 0x%X\n", status);
		//scanf_s("%c", &x, 1);
        if (FAILED(status)) 
            goto exit;
...
const BSTR methodName = SysAllocString(MADBUS_MAD_CONTROL_METHOD_1);
VARIANT funcParam;
//char x;
...
const BSTR methodName = SysAllocString(MADBUS_MAD_CONTROL_METHOD_2);
VARIANT funcParam;
//char x;
...
const BSTR methodName = SysAllocString(MADBUS_MAD_CONTROL_METHOD_3);
VARIANT funcParam;
//char x;
