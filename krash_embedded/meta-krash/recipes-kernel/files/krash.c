...
{
    PINFO("reuseFree(): ptr=%px\n", ptr);
    kfree_sensitive(ptr);
    freeSomething(ptr);
    memset(ptr, 0x78, 100);
    PINFO("reuseFree(): kfree(ptr), memset(ptr) return!");
}
...
void freePage(void *ptr)
{
    ...
    //pPage = pfn_to_page(__pa(ptr) >> PAGE_SHIFT);
    pPage = virt_to_page(ptr);
    PINFO("freePage() ptr=%px, v2p_page=%px\n", ptr, pPage);
...
void hangEvent(krash_private *priv)
{
    struct task_struct *pThread;
    int io_f = 0;
    
    PINFO("hangEvent()\n");
    init_waitqueue_head(&priv->IO_q);   
    priv->arg = 3;
    pThread = kthread_create(KrashFaultThread, (void *)priv, "%s", "KrEvent-hang");
    if (!IS_ERR(pThread))
...
{
    struct task_struct *pThread;
    char ThreadName[] = "Krdeadlock";

    PINFO("deadLock()\n");
    
    spin_lock_init(&priv->spinlock1);
    spin_lock_init(&priv->spinlock2);
    spin_lock(&priv->spinlock1);

    priv->arg = 2;
    pThread = kthread_create(KrashFaultThread, (void *)priv, "%s", ThreadName);
    if (!IS_ERR(pThread))
        {
        wake_up_process(pThread);
        schedule();
        PINFO("deadLock()... pThread=%px name:%s\n", pThread, ThreadName);
        }
...
void overrunStack(void)
{
    ...
    PINFO("overrunStack() recursion... ");
    overrunStack();
    PINFO("overrunStack() exit\n");
...
void trashStack(void)
{
    ...
    PINFO("trashStack() exit\n");
    
    return;
}

noinline __visible int process_krash(int knum, krash_private *priv)
{
    //struct task_struct *pThread;
    int rc=0;
    ...
    switch(knum)
        ...
        default:
            PINFO("process_krash() invalid/unimplemented krash request\n");
            rc = -EINVAL;
            break;
        }
...
noinline __visible long krash_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
